...
output "guardduty_organization_configurations_datasources" {
  ...
  value       = { for k, v in aws_guardduty_organization_configuration.guardduty_organization_configurations : k => one(v.datasources) if v.datasources != null && length(v.datasources) > 0 }
}
